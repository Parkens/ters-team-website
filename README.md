...
Репозиторий проекта по обеспечению доступности, стабильности и безопасности сайта научной группы **www.ters-team.com** при доступе из разных регионов (США/Европа, РФ и КНР) с использованием reverse proxy и CDN/edge-подходов.
...
### Основной вывод
Данная работа не является CDN-задачей, а задачей обеспечения доставки SaaS-контента в условиях нестабильной сетевой маршрутизации и DPI-фильтрации.
...
В данном проекте стабильность была достигнута не через CDN,
а через **контроль транспортного слоя и отказ от "умных" оптимизаций**.

### Примечание о Cloudflare
Хотя итоговая инфраструктура физически МОЖЕТ использовать Anycast-узлы Cloudflare
(через PaaS-провайдера Render), Cloudflare **не участвует в управлении трафиком**:

- DNS не делегирован Cloudflare
- proxy / rules / workers не используются
- QUIC не является обязательным транспортом

Таким образом Cloudflare выступает исключительно как транзитный edge-провайдер,
а не как архитектурный компонент системы.
...
- **Редиректы**: канонизация (`www`) выполняется на proxy для предсказуемости. В регионах с высоким RTT (КНР) возможна дополнительная задержка - допустимый компромисс.
